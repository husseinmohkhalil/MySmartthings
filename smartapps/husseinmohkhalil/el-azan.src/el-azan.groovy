...
preferences {
    section("Which device(s) to Play Sound ") {
        input(name: "targets", type: "capability.musicPlayer", title: "Target Google Home Speakers or Groups", multiple: true, required: true)
    }
}
...
def GoAzan(){
try{
  def nextAzanTime = GetNextAzanTimeInSeconds();
...
def GetNextAzanTimeInSeconds() {
    ...
    try {
        httpGet(params) {
            ...
            log.debug "allPrayingTimes ${allPrayingTimes}"


            def nextPrayTime = allPrayingTimes.min()

            log.debug "nextPrayTime ${nextPrayTime}"
